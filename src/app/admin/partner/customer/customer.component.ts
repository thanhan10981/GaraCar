...
import { KhachHang, NhanVien } from '../../model/model.component';
...
export class CustomerComponent implements OnInit {
  ...
  searchKeyword = '';
  selectedOption = 'code';
  selectedOptionLabel = 'Mã/Tên/Điện thoại';
  showDropdown = false;
  showForm = false;
  showFormAdd = false;
  ...
  selectedTrangThai = 'Đang hoạt động';
  ...
  constructor( private employeeService: EmployeeService,private http: HttpClient, private cdRef: ChangeDetectorRef,private fb: FormBuilder, private customerService: CustomerService) {
  }
  ...
  this.customer.NguoiTao = this.userName;
  const now = new Date();
...
ngOnInit(): void {
  ...
  this.employeeService.getAll().subscribe({
    next: data => this.nhanVienList = data,
    error: err => console.error('❌ Lỗi lấy nhân viên:', err)
  });
}
//cập nhật khách hàng
...
private getEmptyCustomer(): KhachHang {
 ...
 SelectedCustomer: any = {};
  ...
  searchOptions = [
    { label: 'Mã/Tên/Điện thoại', value: 'code' },
...
onTimeChange() {
  if (this.selectedTime === 'custom') {
    ...
    const isoDate = `${yyyy}-${mm}-${dd}`;  // "2025-05-23"

    // Gán về string (đúng format của input[type=date])
...
filterCustomers(): void {
  ...
  if (this.selectedCreator) {
    params = params.set('creator', this.selectedCreator);
  }
  if (this.selectedLoaiKhach && this.selectedLoaiKhach !== 'tatca') {
    params = params.set('loaiKhach', this.selectedLoaiKhach);
  }
  if (this.selectedTrangThai && this.selectedTrangThai !== 'tatca') {
    params = params.set('trangThai', this.selectedTrangThai);
  }
  if (this.searchKeyword) {
    params = params.set('search', this.searchKeyword);
    params = params.set('searchType', this.selectedOption);
  }
  if (this.fromDate) {
