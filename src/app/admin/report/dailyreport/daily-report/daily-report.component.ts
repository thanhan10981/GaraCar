...
import {Component,OnInit,ElementRef,HostListener,ViewChild,Input,SimpleChanges,} from '@angular/core';
...
// hàm xử lý lọc loại thu chi
onLoaiThuChiChanged(value: string) {
  this.loaiThuChiFilter = value;
  this.loadDataFromApi();
}
...
loadDataFromApi() {
  ...
  // xử lý thời gian (giữ nguyên code cũ của bạn)
  if (!this.currentDateFilter && !this.dateRangeFilter) {
 const now = new Date();
const vnToday = new Date(now.getTime() + 7 * 60 * 60 * 1000); // giờ VN

params.ngay = vnToday.toISOString().split('T')[0]; // Chỉ lấy phần ngày 'yyyy-MM-dd'
params.tuGio = '00:00';
params.denGio = '23:59';
  ...
  const d = new Date(this.currentDateFilter);
  d.setUTCHours(17, 0, 0, 0); // xử lý nếu user chọn từ datepicker theo giờ VN
  params.ngay = d.toISOString();
  params.tuGio = this.timeFilter?.tuGio;
  params.denGio = this.timeFilter?.denGio;
  ...
  // lọc khác
  if (this.khachHangKeyword) params.khachHang = this.khachHangKeyword;
  if (this.nhanVienFilter) params.nhanVien = this.nhanVienFilter;
  if (this.nguoiTaoHienThi) params.nguoiTao = this.nguoiTaoHienThi;
  if (this.phuongThucFilter) params.phuongThucThanhToan = this.phuongThucFilter;
  if (this.loaiThuChiFilter) {params.loaiThuChi = this.loaiThuChiFilter;}

  // 👉 GỌI API TUỲ THEO selectedLoaiBaoCao
  if (this.selectedLoaiBaoCao === 'Thu chi') {
    this.hoadonService.getThuChiTongHop(params).subscribe({
      next: (res) => {
        const data = res.DanhSach as any[];
        data.forEach(d => d.ThoiGian = new Date(d.ThoiGian));

        this.tongSoHoaDon = res.TongSoPhieu;
        this.tongTienTatCa = res.TongTien;
        this.tongTien = res.TongTien;
        this.tongTienThu = res.TongTienThu;
        this.tongTienChi = res.TongTienChi;
        this.soPhieuThu = res.SoPhieuThu;
        this.soPhieuChi = res.SoPhieuChi;
        this.tongSoHoaDon = res.TongSoPhieu;
      ...
      error: (err) => console.error('Lỗi thu chi:', err)
    });
  } else {
    // còn lại là hóa đơn
    if (this.selectedLoaiBaoCao !== 'Tổng Hóa Đơn') {
      params.loai = this.selectedLoaiBaoCao === 'Hóa đơn bán hàng' ? 'bán hàng' : 'sửa chữa';
    }
    ...
    this.hoadonService.getTongHopHoaDon(params).subscribe({
      ...
      error: (err) => console.error('Lỗi hóa đơn:', err)
    });
  }
}
